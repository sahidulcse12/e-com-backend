...
module.exports.updateProductById = async (req, res) => {
    ...
    form.parse(req, (err, fields, files) => {
        ...
        const updateProduct = _.pick(fields, ["name", "description", "price", "category", "quantity"]);
        _.assignIn(product, updateProduct);
    ...
    for (let key in filters) {
        if (filters[key].length > 0) {
            if (key === 'price') {
                ...
                //args['price'] = args er moddhe price name a key create korlam
                args['price'] = {
                    $gte: filters['price'][0],
                    $lte: filters['price'][1]
                }
